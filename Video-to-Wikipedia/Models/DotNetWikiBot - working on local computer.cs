// DotNetWikiBot Framework 2.93 - bot framework based on Microsoft .NET Framework 2.0 for wiki projects
...
namespace DotNetWikiBot
{
	...
	public class Site
	{
        private ILog logger = log4net.LogManager.GetLogger(typeof(Site));
		...
		public void GetMediaWikiMessagesEx(bool forceLoad)
		{
			...
			string src = File.ReadAllText(filePathName, Encoding.UTF8);
			doc.LoadXml(Bot.RemoveUnnecessaryDeclarations(src));
		...
		/// <summary>This internal function loads XML from specified URI, makes XPath query and
	...
	public class PageList
	{
		...
		public void SaveXMLDumpToFile(string filePathName)
		{
			...
			rawXML = Bot.RemoveUnnecessaryDeclarations(rawXML).Replace("\n", "\r\n");
			if (File.Exists(filePathName))
				File.Delete(filePathName);
	...
	public class Bot
	{
		...
		public static readonly Version version = new Version("2.93");
		...
		/// <summary>This internal function removes DOCTYPE and XML namespace declarations from
		/// XML/XHTML document.
		/// For details see: http://www.w3.org/blog/systeam/2008/02/08/w3c_s_excessive_dtd_traffic
		/// </summary>
		/// <returns>Returns document without unnecessary declarations.</returns>
		public static string RemoveUnnecessaryDeclarations(string xmlText)
		{
			if (xmlText.StartsWith("<!DOCTYPE"))
				xmlText = xmlText.Substring(xmlText.IndexOf('>') + 1);
			int firstSpacePos = xmlText.IndexOf(' ');
			int firstCloseTagPos = xmlText.IndexOf('>');
			if (firstSpacePos != -1 && firstCloseTagPos != -1 && firstSpacePos < firstCloseTagPos)
				xmlText = xmlText.Remove(firstSpacePos, firstCloseTagPos - firstSpacePos);
			return xmlText;
		}
